...
- if subscription has `excludeKeywords`, any matching keyword in the description blocks the listing entirely.
...
Every Telegram delivery to a user is written to `TelegramDeliveryLog` with the user, subscription name when available, time, delivery purpose, and result. If a send fails, the admin receives a Telegram notification with the user label, chat ID, purpose, and failure reason.
